...
public class Reader
{
    ...
    /**
     * Starts reading from the beginning of the file. Keeps reading single types
     ...
     * not called on an appropriate index. A list of values is encoded as
     * l<contents>e . The contents consist of the bencoded elements of the list,
     * in order, concatenated. A list consisting of the string "spam" and the
     * number 42 would be encoded as: l4:spami42ee. Note the absence of
    ...
    /**
     * Reads in a bytestring strating at the current position. Throws an error
     * if not possible. A byte string (a sequence of bytes, not necessarily
     * characters) is encoded as <length>:<contents>. The length is encoded in
     * base 10, like integers, but must be non-negative (zero is allowed); the
     * contents are just the bytes that make up the string. The string "spam"
     * would be encoded as 4:spam. The specification does not deal with encoding
     * of characters outside the ASCII set; to mitigate this, some BitTorrent
     * applications explicitly communicate the encoding (most commonly UTF-8) in
     * various non-standard ways. This is identical to how netstrings work,
     * except that netstrings additionally append a comma suffix after the byte
     * sequence.
     *
     * @return BByteString
     * @throws Exception
    ...
    /**
     * Reads in a dictionary. Each dictionary consists of N bytestrings mapped
     * to any other value. Example: d3:foo3:bare == ({foo, bar}) A dictionary is
     * encoded as d<contents>e. The elements of the dictionary are encoded each
     * key immediately followed by its value. All keys must be byte strings and
     * must appear in lexicographical order. A dictionary that associates the
     * values 42 and "spam" with the keys "foo" and "bar", respectively (in
     * other words, {"bar": "spam", "foo": 42}), would be encoded as follows:
     ...
     * values are encoded by prefixing the number with a minus sign. The number
     * 42 would thus be encoded as i42e, 0 as i0e, and -42 as i-42e. Negative
